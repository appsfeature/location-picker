package com.location.picker.activity;
...
public class LocationPickerActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (LocationPicker.getInstance().isEnableTranslucentStatus() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
...
//                getSettingsLocation();
    ...
    private class GetAddressFromLatLng extends AsyncTask<Double, Void, LocationPickerDetail> {
        ...
        @Override
        protected LocationPickerDetail doInBackground(Double... doubles) {
            try {
                ...
                if (addresses != null && addresses.size() > 0) {
                    ...
                    if (address != null)
                        mLocationDetail.setAddressLine2(address);
                    sb.append(address).append(" ");


                    String city = addresses.get(0).getLocality();
                    if (city != null)
                        mLocationDetail.setCity(city);
                    sb.append(city).append(" ");


                    String state = addresses.get(0).getAdminArea();
                    if (state != null)
                        mLocationDetail.setState(state);
                    sb.append(state).append(" ");


                    String country = addresses.get(0).getCountryName();
                    if (country != null)
                        mLocationDetail.setCountry(country);
                    sb.append(country).append(" ");

                    String postalCode = addresses.get(0).getPostalCode();
                    if (postalCode != null)
                        mLocationDetail.setPostalCode(postalCode);
